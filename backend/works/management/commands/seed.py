...
class Command(BaseCommand):
    ...
    def handle(self, *args, **opts):
        ...
        # works
        work_objs = []
        for data in SAMPLE_WORKS:
            obj, _ = Work.objects.get_or_create(title_ko=data['title_ko'], defaults=data)
            work_objs.append(obj)
        self.stdout.write(self.style.SUCCESS(f'작품 {len(work_objs)}개 준비 완료'))
        ...
        # records
        sample_records = [
            (work_objs[0], 9.5, '재난과 판타지, 성장의 요소가 자연스럽게 어우러진 작품.\n작화도 너무 예쁘고 음악이 정말 한 이야기의 감정선을 잘 담아내어 여운이 오래 남았다.'),
            (work_objs[1], 8.5, '몇 번을 다시 봐도 좋은 작품. 치히로의 성장이 인상적이다.'),
            (work_objs[2], 9.0, '시즌 3도 기대를 저버리지 않았다. 카즈마 일행 만세.'),
            (work_objs[3], 9.0, '청춘 그 자체. 매 경기마다 손에 땀을 쥐게 한다.'),
        ]
        for work, rating, content in sample_records:
